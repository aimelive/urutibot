...
public class LangChainConfig {
    ...
    ChatMemoryProvider chatMemoryProvider(DurableChatMemoryGateway gateway,
                                          ChatAuthContext authContext,
                                          AnonChatMemoryRegistry anonRegistry) {
        return memoryId -> {
            String key = memoryId.toString();
            boolean authenticated = authContext.get(key).isPresent();
            return authenticated
                    ? new DurableChatMemory(key, PROMPT_WINDOW_MESSAGES, gateway)
                    : anonRegistry.getOrCreate(key, PROMPT_WINDOW_MESSAGES);
        };
    }
}
