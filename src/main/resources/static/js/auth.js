(function () {
  ...
  function activateTab(which) {
    ...
    modal
      .querySelectorAll("[data-auth-error]")
  ...
  async function logout() {
    // Wipe the caller's persisted chat history server-side BEFORE clearing
    // the JWT - the endpoint requires the bearer token to identify the user.
    // Anonymous chats are ephemeral so there is nothing else to clean up.
    // Fire-and-forget: a network failure shouldn't block sign-out.
    try {
      await api.delete("/api/chatbot/conversation");
    } catch (e) {
  ...
  function renderInlineAuthPrompt(container, opts) {
    ...
    wrap
      .querySelector('[data-inline-auth="login"]')
      .addEventListener("click", () => {
        activateTab("login");
        openModal("auth");
      });
    wrap
      .querySelector('[data-inline-auth="register"]')
      .addEventListener("click", () => {
        activateTab("register");
        openModal("auth");
      });
  ...
  const EYE_OPEN =
    '<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z" fill="none" stroke="currentColor" stroke-width="1.8"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="1.8"/></svg>';
  const EYE_OFF =
    '<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M3 3l18 18" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M10.6 6.2A10.6 10.6 0 0 1 12 6c6.5 0 10 6 10 6a17.7 17.7 0 0 1-3.2 3.9M6.2 7.6A17.7 17.7 0 0 0 2 12s3.5 6 10 6c1.4 0 2.6-.2 3.7-.6" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M9.5 9.5a3 3 0 0 0 4.2 4.2" fill="none" stroke="currentColor" stroke-width="1.8"/></svg>';
