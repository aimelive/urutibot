(function () {
  ...
  const state = {
    ...
    // Anonymous-only tab-scoped memoryId; null while authenticated.
    anonMemoryId: null,
  };
  ...
  // Anonymous visitors need a stable per-tab key for the server's in-memory
  // chat window. Generated fresh on each page load - never persisted to
  // localStorage - so a refresh starts a brand-new conversation.
  if (!authPresent()) {
    state.anonMemoryId = uuid();
  }
  ...
  function setAuth(auth) {
    const wasAuth = authPresent();
    state.auth = auth;
    if (auth) writeJson(STORAGE.AUTH, auth);
    else writeStr(STORAGE.AUTH, null);

    // Auth-state transition: sign-in drops the anonymous tab key (the new
    // conversation is server-keyed by userId); sign-out generates a fresh
    // anon key so the post-logout window doesn't bleed into the prior
    // signed-in context.
    if (wasAuth !== authPresent()) {
      state.anonMemoryId = authPresent() ? null : uuid();
    }
  ...
  /**
   * Returns the tab-scoped memoryId for anonymous requests, or null when
   * authenticated. Authenticated chats are keyed server-side by userId.
   */
  function getAnonMemoryId() {
    return state.anonMemoryId;
  }
  /** Generates a fresh anonymous tab key - used by "New chat" while signed out. */
  function rotateAnonMemoryId() {
    if (!authPresent()) {
      state.anonMemoryId = uuid();
    }
    return state.anonMemoryId;
  ...
  const api = {
    get: (path, opts) =>
      request(path, Object.assign({}, opts, { method: "GET" })),
    post: (path, body, opts) =>
